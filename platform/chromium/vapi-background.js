...
vAPI.Tabs = class {
    constructor() {
        ...
        // Ensure the tab id is a valid one before propagating event to
        // client code. "Invalid" yab id can occur when a browser chrome
    ...
    async create(url, details) {
        ...
        if ( details.popup !== undefined && vAPI.windows instanceof Object ) {
            const createDetails = { url, type: details.popup };
...
vAPI.setIcon = (( ) => {
    ...
    (( ) => {
        ...
        const onLoaded = function() {
            for ( const img of imgs ) {
                ...
                if ( img.r.naturalWidth === 0 ) { return; }
                if ( img.r.naturalHeight === 0 ) { return; }
            }
    ...
    return async function(tabId, details) {
        ...
        const { parts, src, badge, color } = details;

        if ( browserAction.setIcon !== undefined ) {
            if ( parts === undefined || (parts & 0b0001) !== 0 ) {
                browserAction.setIcon(
                    Object.assign({ tabId: tab.id, path: src })
                );
            }
            if ( (parts & 0b0010) !== 0 ) {
...
vAPI.Net = class {
    ...
    async benchmark() {
        if ( typeof µMatrix !== 'object' ) { return; }
        const requests = await µMatrix.loadBenchmarkDataset();
...
vAPI.browserData.clearCache = function(callback) {
    browser.browsingData.removeCache({ since: 0 }, callback);
};
...
vAPI.cookies = {
    start: function() {
        ...
        browser.cookies.onChanged.addListener(changeInfo => {
            if ( changeInfo.removed ) {
                if (
                    reallyRemoved[changeInfo.cause] &&
                    typeof this.onRemoved === 'function'
                ) {
    ...
    getAll: function() {
        return webext.cookies.getAll({});
    },

    remove: function(details) {
        return webext.cookies.remove(details);
    },
};
